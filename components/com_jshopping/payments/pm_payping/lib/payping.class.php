...
Class Payping
{
	...
	*/
	...
	function prepPayment( $order )
    {
		...
		{
			$link = JRoute::_(JURI::root(). "index.php?option=com_jshopping&controller=checkout&task=step5" );
			$app->enqueueMessage('<h3>' . _JSHOP_PM_PAYPING_PAPINGCLASS_CONFIG_ERROR . '</h3>', 'Error');
			$app->redirect($link);
		}
		else{
			...
			try 
			{
				...
				if ($err)
				{
					...
					$app->enqueueMessage('<h3>' . _JSHOP_PM_PAYPING_PAPINGCLASS_NOT_SUCCESSFUL_ERROR . "<br /> cURL Error #:" . $err . '</h3>', 'Error');
					$app->redirect($link);
				}
				else
				{
					if ($header['http_code'] == 200) {
						...
						{
							$link = JRoute::_( "index.php?option=com_jshopping&controller=checkout&task=step5" );
							$app->enqueueMessage('<h3>' . _JSHOP_PM_PAYPING_PAPINGCLASS_NO_CODE_ERROR . '</h3>', 'Error');
							$app->redirect($link);
						}
					} elseif ($header['http_code'] == 400) {
						$link = JRoute::_( "index.php?option=com_jshopping&controller=checkout&task=step5" );
						$app->enqueueMessage('<h3>' . _JSHOP_PM_PAYPING_PAPINGCLASS_NOT_SUCCESSFUL_ERROR . implode('. ', array_values (json_decode($response, true))) . '</h3>', 'Error');
						$app->redirect($link);
					} else {
						$link = JRoute::_( "index.php?option=com_jshopping&controller=checkout&task=step5" );
						$app->enqueueMessage('<h3>' . _JSHOP_PM_PAYPING_PAPINGCLASS_NOT_SUCCESSFUL_ERROR . $this->getGateMsg($header['http_code']). '(' . $header['http_code'] . ')' . '</h3>', 'Error');
						$app->redirect($link);
					}
				}
			} catch (Exception $e) {
				$link = JRoute::_("index.php?option=com_jshopping&controller=checkout&task=step5");
				$app->enqueueMessage('<h3>' . _JSHOP_PM_PAYPING_PAPINGCLASS_NOT_SUCCESSFUL_ERROR . '<br />' . $e->getMessage() .'</h3>', 'Error');
				$app->redirect($link);
			}
		}
    }
