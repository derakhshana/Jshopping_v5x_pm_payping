...
class pm_payping extends PaymentRoot
{
	...
	function showAdminFormParams($params)
	{
		...
		{
			$params['returnUrl'] = JURI::root() . "index.php?option=com_jshopping&controller=checkout&task=step7&act=return"; 
		}
	...
	function checkTransaction($pmconfigs, $order, $act)
	{
		...
		$Inputs = JFactory::getApplication()->input->getArray(
			array('refid' => 'STRING',
				  ...
				  'cardnumber' => 'STRING',
				  'cardhashpan' => 'STRING'));
		$orderPmData = $order->getPaymentParamsData();
		if ($Inputs['code'] != $orderPmData['payment_code'])
		{
			return array(0, _JSHOP_PM_PAYPING_ENDFORM_PAYMENT_PAYMENT_CODE_NOT_MATCH_ERROR);
		}
		...
		return array($result['rescode'], $result['restext'] . '. ' . JText::_('JSHOP_ORDER_NUMBER') . ': ' . $order->order_number . ' ' . _JSHOP_PM_PAYPING_PAPINGCLASS_REFERAL_CODE_LABLE . ': ' . $Inputs['refid'], $transaction, $transactiondata);
	}
	...
	{    
        ...
        $payping1->ReturnPath = $pmconfigs['returnUrl'] . '&js_paymentclass=' . $this->pm_method->payment_class;
        ...
        <body>
			<table align="center" >
				...
				<tbody align="center">
					...
					<td>
						<b>
						<?php echo JText::_('JSHOP_ORDER_NUMBER') . ': ' ?>
						</b>
